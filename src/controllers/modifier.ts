...
const addItems = async ({ body }: Request, res: Response) => {
  try {
    ...
    const createdModifier = await insertModifier(body)
...
const updateItems = async ({ params, body }: Request, res: Response) => {
  try {
    ...
    const updatedModifier = await updateModifier(Number(params.id), body)
...
const removeItems = async ({ params }: Request, res: Response) => {
  try {
    const result = await deleteModifier(Number(params.id))
