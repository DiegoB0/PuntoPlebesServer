...
const getReports = async (period: 'day' | 'week' | 'month'): Promise<ReportData[]> => {
  try {
...
const getStatistics = async () => {
  try {
    ...
    const { data: salesByPeriod, error: salesByPeriodError } = await supabase
      .rpc('get_sales_by_period');

    if (salesByPeriodError) throw new Error('FAILED_TO_FETCH_SALES_BY_PERIOD');

    // 4. Total sales per product
    const { data: totalSalesPerProduct, error: totalSalesPerProductError } = await supabase
      .rpc('get_total_sales_per_product');

    if (totalSalesPerProductError) throw new Error('FAILED_TO_FETCH_TOTAL_SALES_PER_PRODUCT');

    // 5. Revenue distribution by product
    const { data: revenueDistribution, error: revenueDistributionError } = await supabase
      .rpc('get_revenue_distribution');

    if (revenueDistributionError) throw new Error('FAILED_TO_FETCH_REVENUE_DISTRIBUTION');
...
export { insertOrder, getOrders, getOrderById, updateOrder, deleteOrder, getReports, getStatistics }
